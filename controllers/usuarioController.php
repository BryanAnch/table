<?php
    ...
    switch ($_GET["op"]) {
        ...
        case 'insertar':
              ...
              $estado = isset($_POST["estado"]) ? trim($_POST["estado"]) : 1;//trim quita los espacios en blanco a la hora de guardar los datos 
              //$clave=randomPassword();
              $clavehash = hash('SHA256', trim($password));// hash incriptar los datos sha256 tiene que ver los bits
                  $usuario = new Usuario();
                  $usuario->setEmail($email);
                  $encontrado = $usuario->verificarExistenciaDb();// modelo es donde esta esta direccion 
                  if ($encontrado == false) {
                      $usuario->setEmail($email);
