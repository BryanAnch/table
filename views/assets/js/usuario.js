...
function limpiarForms() {
  ...
  */
  $('#usuario_add').on('submit', function (event) {
    event.preventDefault();//no se actualiza la pagina
    $('#btnRegistar').prop('disabled', true);//hace que si el usuario presiona el boton de registro solo cuenta uno y asi no envia varias informacio
    var formData = new FormData($('#usuario_add')[0]);// recolecta cada elemtos y los haces datos llama al formurario
    $.ajax({
      url: '../controllers/usuarioController.php?op=insertar',
      type: 'POST',//llamdado post
      data: formData,
      contentType: false,
      processData: false,
      success: function (datos) { // despues de que volvio que resivio datos
        switch (datos) {
          case '1':
            toastr.success(
              'Usuario registrado' // se guardo bien y muestre una pantallita de exito
            );
            $('#usuario_add')[0].reset();
